...
public class QuotationGenerateController implements Initializable {
    ...
    @FXML private TableView<QuotationDTO> quotationTable;
    @FXML private TableColumn<QuotationDTO, String> colQuotationId;
    @FXML private TableColumn<QuotationDTO, String> colClientId;
    @FXML private TableColumn<QuotationDTO, String> colDescription;
    @FXML private TableColumn<QuotationDTO, Double> colAmount;
    @FXML private TableColumn<QuotationDTO, LocalDate> colDate;

    private final QuotationModel quotationModel = new QuotationModel();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        quotationTable.setOnMouseClicked(event -> {
            if (event.getClickCount() == 1) {
                QuotationDTO selected = quotationTable.getSelectionModel().getSelectedItem();
    ...
    public void btnSave(ActionEvent actionEvent) {
        try {
            ...
            boolean isSaved = QuotationModel.saveQuotation(dto);
    ...
    private void populateFieldsFromDTO(QuotationDTO dto) {
    ...
    private void loadQuotationData() throws SQLException {
        ObservableList<QuotationDTO> list = FXCollections.observableArrayList(quotationModel.getAllQuotations());
        quotationTable.setItems(list);
    }

    private void generateQuotationID() {
        try { txtQuotationId.setText(QuotationModel.generateNextQuotationID()); }
    ...
    public void btnQuotationReportJasper(ActionEvent actionEvent) {
        QuotationDTO sel = quotationTable.getSelectionModel().getSelectedItem();
        if (sel == null) { new Alert(Alert.AlertType.WARNING, "Select a quotation.").show(); return; }
        try {
            InputStream jasperStream = getClass().getResourceAsStream("/reports/QuotationReport.jasper");
            JasperReport report = (JasperReport) JRLoader.loadObject(jasperStream);
            Map<String,Object> params = Map.of(
                    "quotation_id", sel.getQuotationId(),
                    "client_id",    sel.getClientId(),
                    "description",  sel.getDescription(),
                    "amount",       sel.getAmount()
            );
            JasperPrint print = JasperFillManager.fillReport(report, params, new JRBeanCollectionDataSource(List.of(sel)));
            JasperViewer.viewReport(print, false);
        } catch (JRException e) {
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR, "Report failed: " + e.getMessage()).show();
        }
    }
